---
title: SEEKER
type: work
date: "2025"
aliases: [seeker]
tags: [work, tool, web, archive]
sources: ["[[diary]]"]
created: 2026-06-22
updated: 2026-06-22
---

# SEEKER

**A small web tool for browsing the strauh.al image archive — point it at a folder URL (or paste filenames) and it pulls the images.**

## Summary
A utility page within the site that takes a folder URL / plain filenames as input and
displays the contents. It belongs to the same DIY ethos Ernest describes in the diary:
"you don't need to learn to code… just get an llm to figure out the rest for you"
(2025-02-13).

## Connections
- [[strauh.al Archive]] — the corpus SEEKER browses.

## Sources
- [[diary]]

<!-- vault-crosslinks:start -->
## Discovered Connections

- [[knowledge/wiki/pages/SEEKER|SEEKER]] — shared language: seeker, filenames, url
- [[knowledge/wiki/concepts/The Spatial Web|The Spatial Web]] — shared language: seeker, web, corpus
- [[knowledge/wiki/maps/Map - Image Archive|Map - Image Archive]] — shared language: filenames, corpus, diary
- [[knowledge/wiki/concepts/The Internet as Confidant|The Internet as Confidant]] — shared language: diary, web, get
- [[knowledge/wiki/people/Rei Kawakubo|Rei Kawakubo]] — shared language: page, diary, site
- [[knowledge/wiki/works/The Record Label|The Record Label]] — shared language: diary, summary, sources
- [[knowledge/wiki/people/Piero Scaruffi|Piero Scaruffi]] — shared language: diary, summary, sources
- [[knowledge/wiki/people/Clyfford Still|Clyfford Still]] — shared language: diary, summary, sources
<!-- vault-crosslinks:end -->
