---
title: "religious texts dhammapada"
source_kind: "html"
source_file: "/Users/erneststrauhal/GitHub/strauh.al4/religious_texts/dhammapada.html"
source_relpath: "religious_texts/dhammapada.html"
compiled: "True"
compiled_at: "2026-07-19T02:00:04"
tags: ["strauhal", "source/html"]
---
# religious texts dhammapada

Source: [[media/strauh.al4/religious_texts/dhammapada.html|source file]]

Concepts: [[HTML Source]], [[dhammapada]], [[religious texts]], [[strauh.al Archive]]

## Compiled Page

strauh.al/religious_texts/dhammapada

# [strauh.al](https://strauh.al)/[religious_texts](https://strauh.al/religious_texts)/dhammapada

## chapters

##

##
[x]

## search

##
[x]

## how to use
[x]

- navigate: click & drag to rotate, right-click & drag to pan, scroll to zoom.

- explore words: click a word in the cloud to see its connections.

- explore chapters: click a chapter name on the left to highlight its key words.

- filter by theme: click a color in the compass to highlight connections of that theme. click again to reset.

- read verses: click any reference to read its full text.

- search: use the search bar to find any word.

Processing Dhammapada Data...

- mindfulness & wisdom

- suffering & impermanence

- virtue & the path

- states of being

- attachment & aversion

## Related Local Pages

- None yet

<!-- vault-crosslinks:start -->
## Discovered Connections

- [[knowledge/wiki/pages/philosophical texts|philosophical texts]] — named in this note
- [[knowledge/wiki/pages/religious texts bible|religious texts bible]] — shared language: texts, religious, click
- [[knowledge/wiki/pages/religious texts quran|religious texts quran]] — shared language: texts, religious, click
- [[knowledge/wiki/pages/philosophical texts phenomenology of spirit|philosophical texts phenomenology of spirit]] — shared language: texts, click, chapters
- [[knowledge/wiki/concepts/Art Fills the God-Shaped Hole|Art Fills the God-Shaped Hole]] — shared language: religious, wisdom
- [[knowledge/wiki/maps/Map - 3D Graph|Map - 3D Graph]] — shared language: click, drag, theme
- [[knowledge/wiki/maps/Map - Living Graph|Map - Living Graph]] — shared language: click, drag, zoom
- [[knowledge/wiki/culture/reading-the-dhammapada|Reading - The Dhammapada]] — shared language: dhammapada
<!-- vault-crosslinks:end -->
